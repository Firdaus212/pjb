function result = pjb_func(elv_awal, elv_akhir, inf_sel, spls)
    ...
    data_simulasi = xlsread('datareal','datareal1','A2:D11');
    %real_h0= data_simulasi(:,1);
    %real_ht= data_simulasi(1,2);
    ...
    for xh0=1:sim_t1
        ...
        if real_h0(xh0)==0
            real_h0_new(xh0)=t_hj(xh0-1,(86400));
            vol_0(xh0)=t_vj(xh0-1,(86400));          
        else
        ...
        for x_el=1:length(data_elevasi)
            if round(real_ht,2) == data_elevasi(x_el)
    ...
    %     v_opr(xh0)
        ...
        for e_el=1:length(datae_elevasi)
            ...
            if round(real_ht,1) == round(datae_elevasi(e_el),1)
                index_elt(xh0)=e_el;
            end
        end
        ...
        for e_p=1:length(datae_beban)
            if index_elt(xh0)==index_el0(xh0)
                ...
                if index_elt(xh0)<index_el0(xh0)
                    ...
                    Qeb(xh0,e_p)=mean(datae_debit(index_el0(xh0):index_elt(xh0),e_p));
                end
            end    
        end
    ...
    otpt_mw_m(xh0)=(sum(Em_1(xh0,:))+sum(Em_2(xh0,:))+sum(Em_3(xh0,:))+sum(Em_4(xh0,:)))/(86400);
    ...
    otpt_mw_s(xh0)=(sum(Es_1(xh0,:))+sum(Es_2(xh0,:))+sum(Es_3(xh0,:)))/(86400);
    ...
    outpt((xh0+1),3)=real_ht;
    outpt((xh0+1),4)=t_hj(xh0,(86400));
...
result = [];
for i = 1 : length(outpt(1,:))
   key = convertStringsToChars(outpt(1,i));
   key = strrep(key,' ','_');
   key = strrep(key,'/','_');
   key = lower(key);
   result.(key) = str2double(outpt(2,i));
end
end