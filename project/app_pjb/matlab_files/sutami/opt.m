function [outputArg1] = opt(input)
    ...
    if elevasi_awal==elevasi_akhir
        ...
        for bb=1:length(beban_power)
            ...
            vol_digunakan(bb) = q_out_rata_beban(bb)*time*3600; %time=waktu operasi turbin (1+2+3)
        ...
        %% Operasi Turbin 1 Sutami
            ...
            beban_sutami_sec(cc)=beban_sutami_1(cc)+beban_sutami_2(cc)+beban_sutami_3(cc);
        end
    ...
    %% Output Program
    outputArg1.vol_tersedia = round(vol_tersedia, 4);
    outputArg1.vol_used = round(vol_used, 4);
    outputArg1.volume_sisa = round(volume_sisa, 4);
    outputArg1.mean_beban_sutami_1 = round(mean(beban_sutami_1), 4);
    outputArg1.mean_beban_sutami_2 = round(mean(beban_sutami_2), 4);
    outputArg1.mean_beban_sutami_3 = round(mean(beban_sutami_3), 4);
    outputArg1.sum_beban_sutami_1 = round(sum(beban_sutami_1)/3600, 4);
    outputArg1.sum_beban_sutami_2 = round(sum(beban_sutami_2)/3600, 4);
    outputArg1.sum_beban_sutami_3 = round(sum(beban_sutami_3)/3600, 4);
    outputArg1.beban_total_sutami = round(beban_total_sutami, 4);
    outputArg1.beban_total_wlingi = round(beban_total_wlingi, 4);
    outputArg1.jam_mulai_operasi_today = jam_mulai_operasi_today;
    outputArg1.jam_mati_operasi_wlingi = jam_mati_operasi_wlingi;
    outputArg1.waktu_operasi_wlingi = waktu_operasi_wlingi;
    outputArg1.energi_cascade = round(energi_cascade, 4);
end